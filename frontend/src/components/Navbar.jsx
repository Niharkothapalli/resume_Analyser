...
export default function Navbar({ onOpenHistory }) {
  const [healthData, setHealthData] = useState({ status: 'checking', provider: 'cohere' });
  ...
  useEffect(() => {
    const checkHealth = async () => {
      try {
        ...
        setHealthData({ status: 'offline', provider: 'unknown' });
      }
    };
  ...
  return (
    <nav className="relative z-50 py-5 font-sans border-b border-white/5 bg-[#121214]/40 backdrop-blur-md">
      <div className="max-w-[1300px] mx-auto px-6 flex justify-between items-center">
        ...
        <a href="#" className="group text-decoration-none flex items-center">
          <div className="font-['Syne'] font-extrabold text-2xl tracking-[-1.5px] text-white flex items-start">
            ...
            <span className="font-light text-[0.6rem] tracking-wider text-white/60 ml-1.5 -mt-0.5 uppercase">AI</span>
          </div>
        </a>

        {/* Right-Side Actions - System Status First, History Second */}
        <div className="hidden md:flex items-center gap-3">
          ...
          >
            <span className={`w-2 h-2 rounded-full ${isOnline ? 'bg-[#4ade80] animate-pulse' : 'bg-[#f87171]'}`} />
      ...
      {/* Mobile Menu Drawer */}
      {mobileMenuOpen && (
        <div className="md:hidden px-6 pt-4 pb-6 border-t border-white/5 bg-[#121214]/95 backdrop-blur-xl flex flex-col gap-3 animate-fadeIn">
          <div className="py-2.5 px-4 rounded-full text-xs font-['Syne'] text-center border border-white/10 text-white/80 flex items-center justify-center gap-2">
            <span className={`w-2 h-2 rounded-full ${isOnline ? 'bg-[#4ade80] animate-pulse' : 'bg-[#f87171]'}`} />
            <span>System: {isOnline ? 'Online' : 'Offline'} ({healthData.provider || 'Ready'})</span>
          </div>
          {onOpenHistory && (
            <button
              onClick={() => {
